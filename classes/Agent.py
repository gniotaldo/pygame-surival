...
class Agent:
    ...
    def destroy(self, x: int, y: int):
        if abs(self.position.x - x) < 2 and abs(self.position.y - y) < 2:
    ...
    def place(self,object, x: int, y: int):
        if object is not None and (1 <= abs(self.position.x - x) < 3 or 1 <= abs(self.position.y - y) < 3):
    ...
    def move(self,moveType, map: Map):
        ...
        if moveType == 'W':
            ...
            if next_y >= 0 and (
                                map.map_grid[ceil(self.position.x)][floor(next_y)].under2 is None and 
                                map.map_grid[ceil(self.position.x)][floor(next_y)].under is not None
                                ) and (
                                map.map_grid[floor(self.position.x)][floor(next_y)].under2 is None and
                                map.map_grid[floor(self.position.x)][floor(next_y)].under is not None
                                ):
            ...
            if next_y < self.map._height-1 and (
                                map.map_grid[ceil(self.position.x)][ceil(next_y)].under2 is None and
                                map.map_grid[ceil(self.position.x)][ceil(next_y)].under is not None
                                ) and (
                                map.map_grid[floor(self.position.x)][ceil(next_y)].under2 is None and
                                map.map_grid[floor(self.position.x)][ceil(next_y)].under is not None
                                ):
                if self.map.map_grid[int(ceil(self.position.x))][int(floor(self.position.y))].type == Map.Item.ItemType.Water and (
                self.map.map_grid[int(floor(self.position.x))][int(floor(self.position.y))].type == Map.Item.ItemType.Water) and (
                self.map.map_grid[int(ceil(self.position.x))][int(ceil(self.position.y))].type == Map.Item.ItemType.Water) and (
            ...
            if next_x >= BAR and (
                                map.map_grid[floor(next_x)][ceil(self.position.y)].under2 is None and
                                map.map_grid[floor(next_x)][ceil(self.position.y)].under is not None
                                ) and (
                                map.map_grid[floor(next_x)][floor(self.position.y)].under2 is None and
                                map.map_grid[floor(next_x)][floor(self.position.y)].under is not None
                                ):
                if self.map.map_grid[int(ceil(self.position.x))][int(floor(self.position.y))].type == Map.Item.ItemType.Water and (
                self.map.map_grid[int(floor(self.position.x))][int(floor(self.position.y))].type == Map.Item.ItemType.Water) and (
                self.map.map_grid[int(ceil(self.position.x))][int(ceil(self.position.y))].type == Map.Item.ItemType.Water) and (
            ...
            if next_x < self.map._width-1 and (
                                map.map_grid[ceil(next_x)][ceil(self.position.y)].under2 is None and
                                map.map_grid[ceil(next_x)][ceil(self.position.y)].under is not None
                                ) and (
                                map.map_grid[ceil(next_x)][floor(self.position.y)].under2 is None and
                                map.map_grid[ceil(next_x)][floor(self.position.y)].under is not None
                                ):
                if self.map.map_grid[int(ceil(self.position.x))][int(floor(self.position.y))].type == Map.Item.ItemType.Water and (
                self.map.map_grid[int(floor(self.position.x))][int(floor(self.position.y))].type == Map.Item.ItemType.Water) and (
                self.map.map_grid[int(ceil(self.position.x))][int(ceil(self.position.y))].type == Map.Item.ItemType.Water) and (
